...
{\bf Approximating LRU.} {Since exact LRU is too expensive to implement, we'll have to try something else. We make use of the PTE reference bit (R bit) instead. All R bits are initially 0, and as the processes executes, the R bit is set to 1 for pages that are accessed.}
    \begin{enumerate}
        \item[]{\bf Problem.} {We now lose the information of the exact order of references, which is bad if eviction happens infrequently (ends up degrading to FIFO). Let's say before eviction, we've accessed a lot of pages, and all of their reference bits have been set to 1, so we're basically ending up with a random eviction. }
        \item[]{\bf Possible Solutions to the Problem.} {We try to have a period/timer where we can set all reference bits to 0, or, we can have a larger amount of reference bits (Eg. instead of just 1 bit, we can use 4/5 bits).} 
    \end{enumerate}

{\bf Second Chance Algorithm.} {Same as FIFO, except we look at the reference bit for the page we're about to evict. If the R bit is 0, we replace the page, if the R bit is 1, we clear the R bit and re-queue the page as youngest––then we try finding a page again to evict. The idea is that pages that are used often enough to keep reference bits set will not be replaced.}
    \begin{enumerate}
        \item[]{\bf Note.} {The second chance algorithm can be implemented in practice because the FIFOQueue is easy to implement. *In worst case scenario, this algorithm degrades to FIFO.}
        \item[]{\bf Not Recently Used (NRU) Algorithm.} {We can combine the reference bit with the modify bit to create 4 classes of pages (Eg. prefer clean pages). This variant is called the not recently used algorithm.}
    \end{enumerate}

{\bf Clock Algorithm.} {A practical implementation of the LRU algorithm. The idea is to replace a page that is "old enough" by arranging the physical frames in a circular list. Then you use a clockHandle (a cursor) to go through the circular list. As you go through each page, if the reference bit is off, then we evict the page. If the reference bit is on, then we turn the reference bit off, and check the next page.}
    \begin{enumerate}
        \item[]{\bf Cons.} {When the memory is large, the accuracy of the information degrades because we're not looking at the reference bits enough. *In worst case scenario, this algorithm degrades to FIFO.}
        \item[]{\bf What's the minimum age of a page if R bit is off?} {It would be the size of the clock/queue?} 
    \end{enumerate}

{\bf Recency-Based PR Policy.} {Replacement policies based on recency: Eg. Clock, Second-Chance, LRU.}
    
{\bf Frequency-Based PR Policy.} {Also known as counting-based page replacement policies. The idea is that we will count the number of uses for each page.}
    \begin{enumerate}
        \item[]{\bf Pros.} {An easy algorithm to implement, making it more likely to have available hardware support.}
        \item[1]{\bf Least-Frequently-Used (LFU).} {We replace the page that's used least often. *Problem: pages that are heavily used at one time tend to stick around even when not needed anymore.}
        \item[2]{\bf Most-Frequently-Used (MFU).} {We evict a page that's accessed a maximum number of times. *Problem: favours new pages.}
        \item[]{\bf Note.} {Neither of these 2 algorithms are common as they are poor approximations of Belady's/Optimal cause they don't respect temporal locality.}  
    \end{enumerate}

{\bf Scan-Resistant Algorithms.} {The idea is that we combine recency and frequency so that single-use pages can be replaced quickly (Eg. LRU/k, 2Q, ARC, LIRS, S2Q).}
...
{\bf Problems w/ Partitioning.} {Fixed partitioning can cause internal fragmentation and need for overlays. Dynamic partitioning can cause external fragmentation and problems managing available space.}
    \begin{enumerate}
        \item[]{\bf Note.} {Partitioning in general is not used in any modern systems––the common problems is because processes must be allocated to contiguous blocks of physical memory. That's why we use paging instead of partitioning.}
    \end{enumerate}

{\bf Paging.} {This method eliminates external fragmentation because we will never have a situation where there is an unused hole in physical memory that we can allocate to a process as contiguouity is no longer required. Internal fragmentation is still an issue, but not that bad because its reduced to at most a fraction of one page per memory region that you have.  }
    \begin{enumerate}
        \item[1]{}{First, we logically partition physical memory into equal, fixed-size chuncks (eg. 4KB). Each of these chuncks are called {\bf page frames.} The size of each frame is called a {\bf page size}.}
        \item[2]{}{We then divide up a process' virtual address space into chunks of the same size as a physical frame.} 
    \end{enumerate}

{\bf Random Access Memory (RAM).} {It doesn't matter which address you're accessing, the speed of accessing the address will always be the same.}
...
{\bf Hierarchical Page Table.} {Based on the idea of adding multiple levels of indirection, which is based on the intuition that we don't want to allocate memory for parts of the page table that's not in use.}
    \begin{enumerate}
        \item[]{\bf Note.} {Also known as a multi-level page table, and is where the virtual address is broken into N + 1 pieces. The page frame is located at the bottom level page table. }
        \item[]{\bf Two Level Page Table.}
        \begin{enumerate}
            \item[]{\bf Page Directory Base Register (PDBR).} {Points to the the address where the page directory, or the top level page table is located.}
            \item[]{\bf Page Directory.} {The top level page table containing pagetables. Each page table in the page directory points to a secondary page table, or a bottom page table.}
            \item[]{\bf Page Directory Index.} {The address of a page table in a page directory.}
            \item[]{\bf Secondary Page Table.} {The bottom level page table of a two level page table.} 
            \item[]{\bf Page Table Index.} {The address of a page frame in the secondary page table.}
        \end{enumerate}
    \end{enumerate}
...
{\bf There are 2 ways to handle TLB misses.} {Who is responsible for loading the missing translation into the TLB?}
    \begin{enumerate}
        \item[1]{\bf Hardware Loaded.} {In this scenario the MMU (Eg. Intel Pentium) is able to look up the page table in memory, find the entry that's requested, and load the entry into the TLB. Because the hardware needs to access the page table for the current running process, the OS has to settup the PTBR when swtiching to a new process. The OS is also responsible for building and maintaining page tables in memory. The CPU will read in the page table and perform TLB faults.}
        \begin{enumerate}
            \item[]{\bf Note.} {Hardware reads the page table, but doesn't write to the page table. We only write to the page table as an OS.}
        \end{enumerate}
        \item[2]{\bf Software Loaded.} {Gives us more flexibility because the OS gets to decide the page table format and the PTE format. When there's a TLB miss, we fault into the OS––the exception handler is responsible for looking up the page table to find the appropriate translation to load into the TLB.}
        \begin{enumerate}
            \item[]{\bf Note.} {The software/OS is both reading and writing to the page table.}
        \end{enumerate}
        \item[]{\bf Note.} {Software loaded TLB is much slower than hardware loaded TLB.}
        \item[]{\bf OS.} {Responsible for creating page tables.}  
    \end{enumerate}

Timestamp 6:37

{\bf Managing TLBs.} {*A context switch is 500x more expensive.}
    \begin{enumerate}
        \item[1]{\bf OS.} {OS ensures that TLB and page tables are consistent. When OS changes the protection bits of a PTE, it needs to update the copy of the PTE in the TLB (if it exists).}
        \item[2]{\bf Context Switch.} {We reload the TLB on a process context switch, invalidating all entries––this make context switches expensive (cause there's going to be a bunch of TLB misses at the beginning).}
        \item[3]{\bf Cached PTE Eviction.} {When a TLB misses and a new PTE must be loaded, a cached PTE likely needs to be evicted. Choosing which PTE to evict is called the TLB replacement policy. Implementing in hardware for hardware-loaded TLB is often simple.}   
    \end{enumerate}

{\bf Intel Core i7 Address Translation.} 
    \begin{enumerate}
        \item[1] {} {Look up page table and check the TLB for a matching entry at the same time. If we have a TLB hit, then we cancel page table lookup. }
        \item[]{\bf Note.} {Looks like Intel Core i7 uses a 4-level page table.} 
    \end{enumerate}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.75\linewidth]{Inteli7AddressTrans} 
  % \caption{An example image}
  % \label{fig:example}
\end{figure}
\vspace{10em}

{\bf Paged Virtual Memory.} {The idea of implementing paging. The OS uses main memory as a page cache of all data allocated in the system.}
    \begin{enumerate}
        \item[1]{Initially, pages are allocated from memory. When memory fills up, allocating a page in memory requires some other page to be evicted from memory.}
        \item[2]{Evicted pages go to the {\bf swap} space. Swap is a location on disk.} 
        \item[3]{OS keeps track of use of each physical frame with a {\bf coremap} data structure.} 
    \end{enumerate}

{\bf Swapping.} {The action of moving the contents of a page into disk.}

{\bf Page Fault.} {This happens when the page being accessed is invalid (PTE is invalid). A {\bf page fault handler} deals with page faults and tries to make pages valid.}
    \begin{enumerate}
        \item[1]{\bf Minor Page Fault.} {Happens when disk access is not needed. Eg. first access to a new stack or heap page.}
        \item[2]{\bf Major Page Fault.} {Happens when disk access is needed. Eg. First access to a code or data region, access to evicted pages (remember that evicted pages go to swap)} 
        \item[]{\bf Note.} {Page fault only happens when both the page table and the TLB are invalid for that address translation (TLB and PTE both have a valid/invalid bit).} 
    \end{enumerate}

{\bf Eviction.} {When OS runs out of memory, we need to evict something. A victim fram ei chosen from the {\bf coremape}. The PTE of the page mapped to the frame is set to invalid, if the PTE is cached in TLB, we must also invalidate the TLB entry. The {\bf swap location} of the page is then *stored in the PTE.}
    \begin{enumerate}
        \item[]{\bf What happens when a process accesses a page that's been evicted?} {The invalid PTE will cause a page fault (trap), the trap will run the OS page fault handler, the handler uses the invalid PTE to locate page in swap (on disk). The page is then read into a physical frame, and the PTE is update to point to the page. The process finally resumes.}
        \item[]{\bf Note.} {The page fault is generated by the CPU.}
    \end{enumerate}
...
{\bf Deadlock Prevention.} {The main idea for dealing with deadlock is to break on of the four conditions to avoid deadlock.}
    \begin{enumerate}
        \item[]{\bf The 4 Conditions.} {Mutual exclusion, hold and wait, no pre-emption, and circular wait.}
        \item[1]{\bf Breaking Mutual Exclusion.} {We can break mutex with something called lock-free data structures. (Eg. Use hardware atomic Compare-And-Swap instruction.)} 
        \item[2]{\bf Breaking Hold and Wait.} {We can possibly break hold and wait by locking resources or using the trylock() function.} 
        \item[3]{\bf No Pre-emption.} {We can only break pre-emption if a resource has a rollback state, such as the CPU as you can save context, otherwise it's not feasible to break no pre-emption.}
        \item[4]{\bf Circular Wait.} 
    \end{enumerate}

{\bf Breaking Mutex: Compare-And-Swap (CAS).} {Use CAS instead of locks to break mutex. If you need more than 1 address to update a value to, then a lock-free data structure is not possible.}
    \begin{lstlisting}[language=C, numbers=left, basicstyle=\ttfamily]
    ...
    // Non-blocking Atomic Add
    // If the address val, still contains the old value, then CAS will return true.
    void AtomicAdd(int *val, int a){
        do {
            int old = *val;
...
{\bf Breaking Block and Wait.} {Hold and Wait policy tries to gather all needed resource at the same time, which can lead to long delays especially if one or more resources are not readily avaialble––leading to threads stalling, and the wait time to make any progress in the critical section can increase dramatically.}
    \begin{enumerate}
        \item[1]{\bf Another Problem: Locked Resources.} {We're forced to lock all resources at the beginning, before the resource is actually needed, limiting concurrency, and resulting in blocking many other threads.}
        \item[2]{\bf Another Problem: Resource Requirements.} {We may not know all the resource requirements in advanced.}
        \item[3]{\bf Alternative.} {Some thread libraries offer a trylock() function––you grab a lock if it's avaialable, else we try again later. Deadlock is avoided. Problem: We could have a possible livelock. } 
    \end{enumerate}

    \begin{lstlisting}[language=C, numbers=left, basicstyle=\ttfamily]
    // How we can break hold and wait?
    
    Record reci = get_record(db, pki) // Gets a database record
    reci.lock(); // Lock resource pki 

    int i = get_value(reci, "i", int); // Gets shared int

    // If int < 0, grab resource B (pkb), else grab A (pka)
    Record recx = get_record(db, i<0 ? pkb : pka);
    recx.lock(); // Lock resource either pkb or pka


    int x = get_value(recx, "cost", int);
    inc_value(recx, "views"); // Increment view of x
    recx.unlock();
    set_value(reci, "i", i + x); // Increment cost x by i
    reci.unlock(); 
    \end{lstlisting}

    \begin{lstlisting}[language=C, numbers=left, basicstyle=\ttfamily]
    // Trylock() Function
    // We will keep trying until we have both locks ready at the same time.
    // Deadlock avoided.

    ready = false;
    while (!ready){
        lock(L1);
        if (trylock(L2) == -1) // If L2 is available
            unlock(L1);
        else{
            ready = true;
        }
...
{\bf Livelock.} {A case where all of the threads are not blocked, but at the same time, no progress is being made. }
    \begin{enumerate}
        \item[]{\bf Eg. Trylock().} {In a trylock() threads can get stuck waiting to be ready/acquire multiple locks at the same time.}
        \item[]{\bf Solution.} {Using something called exponential backlog, where each time we want to try acquiring the lock again, we wait for a longer period of time to try again.} 
    \end{enumerate}

{\bf Breaking No Pre-emption.} {No pre-emption means we're force-taking resources from a thread. The main idea is that taking away a resource from another thread is only safe when we can rollback a resource to a previous/saved state.}
    \begin{enumerate}
        \item[]{\bf When is it safe to take a resource away from a thread?} {It depends on the resource. We can take away the CPU, but not a data structure like linked list. The CPU is safe because we save the context when we take the CPU.}
        \item[]{\bf Note.} {Breaking pre-emption isn't really feasible, or it's too compelx to achieve.} 
    \end{enumerate}

{\bf Breaking Circular Wait.} {To break circular wait, we assign a {\bf linear ordering} to resource types and require that a thread holding a resource of one type, R, can only request resources that follow R in the ordering.}
    \begin{enumerate}
        \item[]{\bf Lock Ordering Eg.} {Must lock $R_1$ before $R_2$ before $R_3$.}
    \end{enumerate}

{\bf Dining Philosophers Problems.} {You have a round table of philosophers, there's only 1 chopstick on each side of a philosopher, and to eat they need 2 chopsticks.}

    \begin{lstlisting}[language=C, numbers=left, basicstyle=\ttfamily]
    // Let each philosopher be a thread
    ...
    void philosopher(int pid)
    {
        ...
        while (true){
            think();
            lockAcquire(chopsticks[left]);
            lockAcquire(chopsticks[right]);

            eat();

            lockRelease(chopsticks[right]);
            lockRelease(chopsticks[left]);
        }
    }

    // Would this result in a deadlock? Yes it would.
    // Why?: Because each philosopher is able to acquire locks to chopsticks on their left. If there's 5 chopsticks, and 5 philosophers, and each philosopher already acquire 1 chopstick, then no philosopher can make progress eating as they can't acquire the chopstick on the right.  
    \end{lstlisting}

{\bf Lock Ordering: Dining Philosophers.} {A solution to prevent deadlock by preventing circular wait.}

    \begin{lstlisting}[language=C, numbers=left, basicstyle=\ttfamily]
    struct lock *chopsticks[N];
    void philosopher(int pid){
        int left = pid;
        int right = (pid + 1) % N; // The "circular part"

        // Breaking Circular Wait: Lock Ordering // 
        vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv

        // If left < right, take the left chopstick, else take right.
        int first = (left < right) ? left : right; // L1

        // If left < right, take right chopstick, else take left.
        int second = (left < right) ? right : left; // L2
        ...
        while (true){
            think();
            lockAcquire(chopsticks[first]);
            lockAcquire(chopsticks[second]);

            eat();

            lockRelease(chopsticks[second]);
            lockRelease(chopsticks[first]);
        }
    }
